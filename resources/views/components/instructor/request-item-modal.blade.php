<!-- Add Request Modal -->
<div id="add-modal" class="fixed inset-0 z-50 flex items-center justify-center {{ ($addOpen ?? false) ? '' : 'hidden' }} bg-black/50">
        ...
        <form id="add-form" method="POST" action="{{ route('borrower.request.store') }}" class="space-y-4 p-6">
            ...
            <div>
                ...
                <select name="equipment_id" id="equipment_id" required
                    class="block w-full rounded-lg border border-gray-300 bg-white px-3 py-2 text-sm text-gray-700 focus:border-brand focus:outline-none focus:ring-2 focus:ring-brand/20">
            ...
            <div>
                ...
                <p class="mt-1 text-xs text-gray-500">Must not exceed the available stock shown for the selected equipment.</p>
            </div>
            ...
            <div>
                ...
                <textarea name="remarks" id="remarks" rows="3"
                    class="mt-0 block w-full rounded-lg border border-gray-300 bg-white px-3 py-2 text-sm text-gray-900 focus:border-brand focus:outline-none focus:ring-2 focus:ring-brand/20">{{ old('remarks') }}</textarea>
            </div>

            {{-- Inline error echo so feedback is visible inside the open modal --}}
            @isset($errors)
                @if ($errors->any())
                    <div role="alert" class="rounded-lg border-l-4 border-red-500 bg-red-50 px-3 py-2 text-xs text-red-700">
            ...
            <div class="flex justify-end space-x-3 pt-2">
                <button type="button" data-dismiss="#add-modal"
